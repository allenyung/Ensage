	...
	if ScriptConfig.combo then
		...
		if  #Queue == 0 then
			...
			if me:DoesHaveModifier("modifier_sand_king_epicenter") then
				if distance < 875 and veil_of_discord and veil_of_discord.state == LuaEntityAbility.STATE_READY then
					me:CastAbility(veil_of_discord,target.position)
					sleep(1,100)
				end
			end
			
			if blink and blink.state == LuaEntityAbility.STATE_READY and (_Q.state == LuaEntityAbility.STATE_READY) and distance-1250 <= range and distance > range then
